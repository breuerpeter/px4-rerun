...
uint64_t ts_diff(uint64_t a, uint64_t b) { return a > b ? a - b : b - a; }
...
void log_pose(rerun::RecordingStream& rec, const std::shared_ptr<ulog_cpp::DataContainer>& data) {
  ...
  for (const auto& s : *pos_sub) {
    try {
      ...
      set_vehicle_attitude(att_samples[j].q[0], att_samples[j].q[1], att_samples[j].q[2],
                           att_samples[j].q[3]);
...
void log_setpoint(rerun::RecordingStream& rec,
...
void log_mission(rerun::RecordingStream& rec,
                 const std::shared_ptr<ulog_cpp::DataContainer>& data) {
  ...
  for (const auto& s : *sub) {
    try {
      ...
      try {
        alt_relative = s["altitude_is_relative"].as<int>() != 0;
      } catch (const std::exception&) {
      }
...
void log_text_messages(rerun::RecordingStream& rec,
                       const std::shared_ptr<ulog_cpp::DataContainer>& data) {
  for (const auto& entry : data->logging()) {
    uint8_t severity = static_cast<uint8_t>(entry.logLevel()) - '0';
    log_text(rec, static_cast<int64_t>(entry.timestamp()), entry.message(), severity);
  }
}

void log_all_scalars(rerun::RecordingStream& rec,
                     const std::shared_ptr<ulog_cpp::DataContainer>& data) {
    ...
    for (const auto& sample : *sub) {
      if (first) {
        for (const auto& field : fields) {
          if (field == "timestamp" || field.find("_padding") != std::string::npos) continue;
          try {
      ...
      try {
        ...
        for (const auto& field : numeric_fields) {
          try {
            log_scalar(rec, "timeseries/" + name + "/" + field, ts, sample[field].as<double>());
          } catch (const std::exception&) {
          }
        }
      } catch (const std::exception&) {
...
void fetch_and_log_terrain(rerun::RecordingStream& rec,
                           const std::shared_ptr<ulog_cpp::DataContainer>& data) {
  ...
  auto terrain = terrain_fetcher::fetch_terrain(ref_lat, ref_lon, static_cast<double>(ref_alt),
