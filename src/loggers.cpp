...
void set_vehicle_position(float x, float y, float z) { last_pos = coords::ned_to_zup(x, y, z); }
...
void log_vehicle_pose(rerun::RecordingStream& rec, int64_t timestamp_us) {
  ...
  rec.log("px4/body", rerun::Transform3D::from_translation_rotation(
                          ...
                          rerun::datatypes::Quaternion::from_wxyz(last_quat[0], last_quat[1],
                                                                  last_quat[2], last_quat[3])));

  rec.log("px4/body/frd",
          rerun::Arrows3D::from_vectors({{0.3f, 0, 0}, {0, -0.3f, 0}, {0, 0, -0.3f}})
              .with_colors(
                  {rerun::Color(255, 0, 0), rerun::Color(0, 255, 0), rerun::Color(0, 0, 255)}));
...
void log_velocity(rerun::RecordingStream& rec, int64_t timestamp_us, float x, float y, float z,
...
void log_setpoint_pose(rerun::RecordingStream& rec, int64_t timestamp_us, float x, float y, float z,
                       float yaw) {
  set_timestamp(rec, timestamp_us);
  ...
  rec.log("px4/world/setpoint_yaw_label",
          rerun::Points3D({{pos[0] + fwd[0], pos[1] + fwd[1], pos[2] + fwd[2]}})
...
void log_home_position(rerun::RecordingStream& rec, int64_t timestamp_us, float x, float y,
                       float z) {
  set_timestamp(rec, timestamp_us);
...
void log_mission_item(rerun::RecordingStream& rec, int64_t timestamp_us, float x, float y, float z,
                      uint16_t nav_cmd) {
  set_timestamp(rec, timestamp_us);
...
void log_text(rerun::RecordingStream& rec, int64_t timestamp_us, const std::string& text,
              uint8_t severity) {
  set_timestamp(rec, timestamp_us);
...
void log_scalar(rerun::RecordingStream& rec, const std::string& entity_path, int64_t timestamp_us,
                double value) {
  set_timestamp(rec, timestamp_us);
  rec.log(entity_path, rerun::Scalars(value));
}

void log_sensor_mag(rerun::RecordingStream& rec, int64_t timestamp_us, uint8_t mag_idx, float x,
                    float y, float z) {
  set_timestamp(rec, timestamp_us);
...
void log_mag_cal_samples(rerun::RecordingStream& rec, int64_t timestamp_us, uint8_t mag_idx,
                         const std::vector<std::array<float, 3>>& samples) {
  set_timestamp(rec, timestamp_us);
  std::string path = "px4/body/mag_worker_data/" + std::to_string(mag_idx);
  rec.log(path, rerun::Points3D(samples).with_colors({mag_color(mag_idx)}).with_radii({0.01f}));
}
...
rerun::components::TextLogLevel severity_to_rerun_level(uint8_t severity) {
  switch (severity) {
    case 0:
      return rerun::TextLogLevel::Critical;
    case 1:
    case 2:
    case 3:
    ...
    default:
      return rerun::TextLogLevel::Trace;
  }
}
