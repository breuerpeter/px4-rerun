...
static toff_t tiff_seek(thandle_t h, toff_t off, int whence) {
    ...
    case SEEK_END: m->offset = m->size + off; break;
    }
    return m->offset;
}
static int tiff_close(thandle_t) { return 0; }
static toff_t tiff_size(thandle_t h) {
    return static_cast<TiffMemBuf*>(h)->size;
}

// ── Fetch elevation ──────────────────────────────────────────────────────────

ElevationData fetch_elevation(double lat_min, double lat_max,
                               double lon_min, double lon_max)
{
    ...
    TIFF* tif = TIFFClientOpen("mem", "rm", &membuf,
        ...
        for (uint32_t y = 0; y < h; y += th) {
            for (uint32_t x = 0; x < w; x += tw) {
                ...
                for (uint32_t r = 0; r < copy_h; ++r) {
                    std::memcpy(&elev.elevations[(y + r) * w + x],
                                &tile_buf[r * tw],
                                copy_w * sizeof(float));
                }
            }
...
ImageData fetch_imagery(double lat_min, double lat_max,
                         double lon_min, double lon_max)
{
    std::ostringstream path;
...
std::vector<float> convert_to_local_enu(const ElevationData& elev,
                                         ...
                                         double ref_alt)
{
    ...
    int dbg = n / 2;
    std::cerr << "PROJ geoid: elev=" << heights[dbg]
              << " → wgs84=" << wgs_heights[dbg]
...
std::vector<uint8_t> export_glb(const Mesh& mesh, const ImageData& texture)
{
    ...
    stbi_write_png_to_func(png_write_callback, &png_data,
...
TerrainData fetch_terrain(double ref_lat, double ref_lon, double ref_alt,
